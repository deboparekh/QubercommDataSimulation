...
app.controller('VenueCtrl', ['$scope', '$timeout','$compile', '$http', function($scope, $timeout, $compile, $http){
    ...
    $scope.sunburstGetData = function(){
        $http.get('sunburst.json').then(function successCallback(response){
            $scope.sunburstData = response.data;
        }, function errorCallback(response){
            console.log(response);
        });
    }
    $scope.sunburstGetData();
