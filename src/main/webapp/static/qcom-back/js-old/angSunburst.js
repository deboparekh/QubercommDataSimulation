app.directive('sunburst', function ($window, $http, $routeParams) {
...
function sunburstF(chart) {
 ...
 // d3.select(sElement).select("svg").remove();
  ...
  .style("fill", function(d) { return d.bgColor ? d.bgColor : color((d.children ? d : d.parent).name); })
  .style("opacity", 1)
  .style("stroke", '#fff')
  .on("mouseover", mouseover);
  //.on("click", click);
  
   d3.select(sElement).on("mouseleave", mouseleave);
  ...
  var link = g.append("a").attr("xlink:href",function(d){ 
       ...
       if(d.type=="venue")
        var url="/facesix/web/site/portion/list?sid=";
       else if(d.type=="floor")
        var url="/facesix/web/site/portion/dashboard?spid=";
       else if(d.type=="server")
        var url="/facesix/web/site/portion/dashboard?spid=";
       else
        var url="/facesix/web/site/portion/";
       return url
     })
...
var sunBurstChart = {
    ...
    getJSON: function() {
        ...
        //console.log(urlObj);
        $http({
            url: '/facesix/rest/site/portion/networkdevice/view?sid='+$routeParams.sid,
            //url: 'sunburst1.json',
            method: 'get'}).then(
                function successCallback(response){
                     //console.log(response);
                var json = response.data.floors;
                
                sunburst1.name=response.data.name;
                sunburst1.sid=response.data.sid;  
