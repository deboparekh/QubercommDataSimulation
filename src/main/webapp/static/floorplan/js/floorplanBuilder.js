	...
	floorBuilder = {
		init: function(id){ 
			...
			if(spid != '' && spid != 'null' ){ 
		       ...
		       console.log(spid);
				$.ajax({ 
					url: "/facesix/rest/site/portion/open?spid="+spid,  
		            processData : false,
		            contentType : false,
		            type : 'GET',
					success: function(response) { 
						console.log(response);
						loadingJson = JSON.parse(response);   
						...
						//$('body').append('<div class="alert alert-success fade in alert-dismissable ui-alert" > <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a><div class="alert-inner">'+msg+'</div></div>'); 
					},
					error:function(msg) { 
						//$('body').append('<div class="alert alert-danger fade in alert-dismissable ui-alert" > <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a><div class="alert-inner">'+msg+'</div></div>'); 
					}
				});  
			...
			canvas.on('object:selected',function(){
			  ...
			  var t;
				...
				if (o) { 
					var opacity = o.get('opacity');
					$('input[name="objectOpacity"]').val(opacity); 
					var stroke = o.get('stroke');
					$('input[name="strokeColor"]').val(stroke); 
					var strokeWidth = o.get('strokeWidth');
		...
		changeWidth: function(element){ 
			var newWidth = $(element).val();
			if(newWidth >= 100){
		...
		changeHeight: function(element){ 
			var newHeight = $(element).val();
			if(newHeight >= 100){
		...
		changeBg: function(element){
			var element = element;
			$(element).colorpicker().on('changeColor', function() { 
		...
		changeTxtColor: function(element){
			var element = element;
			$(element).colorpicker().on('changeColor', function() { 
		...
		changeFontSize: function(element){
			var FontSize = $(element).val();
			if(canvas.getActiveObject()){
				canvas.getActiveObject().setFontSize(FontSize);
				canvas.renderAll();
			} 
		},
		drawObjec: function(val){
			canvas.deactivateAllWithDispatch();
			canvas.renderAll();
			floorBuilder.drawObj(val);
		...
		drawObj: function(objType) {
			// Esc key handler
			$(document).keyup(floorBuilder.escHandler); 
			...
			canvas.on('mouse:down', function(o){
				...
				$(document).unbind("keyup", floorBuilder.escHandler);
		...
		sendForward: function() {
			if (canvas.getActiveGroup() !== null) {
				floorBuilder.sendGroupForward(canvas.getActiveGroup(), false);
		...
		sendBackward: function() {
			if (canvas.getActiveGroup() !== null) {
				floorBuilder.sendGroupBackward(canvas.getActiveGroup(), false);
			} else if (canvas.getActiveObject() !== null) {
		...
		sendToFront: function() {
			if (canvas.getActiveGroup() !== null) {
				floorBuilder.sendGroupForward(canvas.getActiveGroup(), true);
			} else if (canvas.getActiveObject() !== null) {
		...
		sendToBack: function() {
			if (canvas.getActiveGroup() !== null) {
				floorBuilder.sendGroupBackward(canvas.getActiveGroup(), true);
			} else if (canvas.getActiveObject() !== null) {
		...
		sendGroupBackward: function(group, bottom) {
		...
		// TODO Fabric.js might do this for us now that we've on version >1.5
		sendGroupForward: function(group, top) {
			// Copy object references
			var sorted = group.objects.slice();
		...
		exportFile: function(fileType) {
			...
			$.ajax({ 
				url: "/facesix/rest/site/portion/floorplan/save",
				...
				success: function(response) {
					...
					spid = response;  
					if (spid!="") {
						var url  = "/facesix/web/site/portion/list?sid="+sid+"&cid="+cid;
						window.location = url;
			...
			// Save file
			// saveAs(blob, "design." + fileType);
		},
		insertSvg: function(url, loader) {
		...
		dtEditText: function(action) {
			...
			if (o && t === 'i-text') {
				switch(a) {
					case 'bold':				
						var isBold = floorBuilder.dtGetStyle(o, 'fontWeight') === 'bold';
						floorBuilder.dtSetStyle(o, 'fontWeight', isBold ? '' : 'bold');
					break;

					case 'italic':
						var isItalic = floorBuilder.dtGetStyle(o, 'fontStyle') === 'italic';
						floorBuilder.dtSetStyle(o, 'fontStyle', isItalic ? '' : 'italic');
					break;

					case 'underline':
						var isUnderline = floorBuilder.dtGetStyle(o, 'textDecoration') === 'underline';
						floorBuilder.dtSetStyle(o, 'textDecoration', isUnderline ? '' : 'underline');
					break;
					canvas.renderAll();
				}
			}
		},
		dtGetStyle: function(object, styleName) {
			return object[styleName];
		},
		dtSetStyle: function(object, styleName, value) {
			object[styleName] = value;
			object.set({dirty: true});
			canvas.renderAll();
		},
		changeOutLine: function(object, styleName, value) {
			object[styleName] = value;
			object.set({dirty: true});
			canvas.renderAll();
		},
		changeStroke: function(ele, action){
			var ele = ele;
			...
			if(obj){
				switch(action) {
					...
					case 'color':
						...
						$(ele).colorpicker().on('changeColor', function() { 
